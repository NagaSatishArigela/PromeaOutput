...
(function($) {
	...
	window.Paging = function(options) {
		...
		options = $.extend({
			...
			pagination_parent: namespace + "-nav-parent",
		...
		var prev_html = '<li class="prev page first '+options.prev+'"><a href="#">이전 페이지</a></li>';
		var next_html = '<li class="next page last '+options.next+'"><a href="#">다음 페이지</a></li>';
		...
		function step(seq)
		{
			...
			$("."+options.pagination_current, container).parent().removeClass(options.pagination_parent);
			$("."+options.pagination_current, container).addClass('hover').removeClass(options.pagination_current);


			$("."+options.pagination+"[current_no="+current_no+"]", container).parent().addClass(options.pagination_parent);
		...
		function make_paging_html()
		{
			...
			for(i=1; i<=page_scale; i++)
			{
				...
				if(container.data('current_no') == page_no)
					result_html += '<li class="'+options.pagination_parent+'"><a href="javascript:;" class="'+options.pagination+' '+options.pagination_current+'" seq="'+i+'" current_no="'+page_no+'">'+page_no+'</a></li>';
				else
					result_html += '<li ><a href="javascript:;" class="'+options.pagination+' hover" seq="'+i+'" current_no="'+page_no+'">'+page_no+'</a></li>';
			}
			...
			return $("<ul class='"+options.viewport+"'>").append(result_html);
		}
